...
class SO3:
    ...
    def exp(cls, phi):
        ...
        # 动态创建单位矩阵，跟随输入数据(phi)的设备(device)和类型(dtype)
    ...
    def log(cls, Rot):
        ...
        phi = cls.vee(Rot - Id)
        # angle = angle # (Redundant line removed)

        # Fix shape for broadcasting
        angle_expanded = angle.unsqueeze(1).unsqueeze(2)

        phi[~mask] = cls.vee(
            (0.5 * angle_expanded[~mask] / angle_expanded[~mask].sin()) * (Rot[~mask] - Rot[~mask].transpose(1, 2)))
        return phi
    ...
    def dnormalize(cls, Rots):
        U, _, V = torch.svd(Rots)
        # [修改点6] 动态创建 Double 精度的单位矩阵
        S = torch.eye(3, device=Rots.device, dtype=torch.double).unsqueeze(0).repeat(Rots.shape[0], 1, 1)
        S[:, 2, 2] = torch.det(U) * torch.det(V)
        return U.bmm(S).bmm(V.transpose(1, 2))
...
# 保持 CPUSO3 存在以防其他代码依赖，但 SO3 类已经可以处理 CPU 情况了
class CPUSO3:
    # tolerance criterion
    TOL = 1e-8
    Id = torch.eye(3)
